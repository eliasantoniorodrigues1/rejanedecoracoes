    ...
    for nome_tema in sorted(os.listdir(FOTOS_DIR)):
        ...
        if os.path.exists(legenda_path):
            with open(legenda_path, 'r', encoding='utf-8') as f:
                ...
                print(f"[INFO] Legendas do tema {nome_tema}: {legendas}")
                capa_nome = legendas.get("nome")
                print(f"[INFO] capa_nome do tema {nome_tema}: {capa_nome}")
                if capa_nome:
                    imagem_capa = os.path.join(
                        "static", "images", "fotos", nome_tema, capa_nome).replace("\\", "/")

        if imagem_capa:
            dados_temas.append({
                'nome': nome_tema.title(),
                'caminho': imagem_capa,
                'rota': f'/tema/{nome_tema.lower().replace(" ", "-")}/',
            })

    return render(request, 'home.html', {'temas': dados_temas})
